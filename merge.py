...
print([f for f in os.listdir(data_folder_path) if f.startswith("data")])

# 2. creates list with files to merge based on name convention
file_list = [data_folder_path + f for f in os.listdir(data_folder_path) if f.startswith('data')]
...
for file in sorted(file_list):
    print(ind)
    ind += 1
    csv_list.append(pd.read_csv(file, encoding="cp1252").assign(File_Name = os.path.basename(file)))
...
csv_merged = csv_merged.sort_values(by=["Stazione", "Inquinante", "Data", "Ora"])

# 6. Single DF is saved to the path in CSV format, without index column
csv_merged.to_csv(save_path + 'APPA data merged.csv', index=False, columns=["Stazione","Inquinante","Data","Ora","Valore","Unità di misura"])
